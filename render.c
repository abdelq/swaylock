...
void render_frame(struct swaylock_surface *surface) {
	...
	int buffer_width = surface->current_buffer->width;
	int buffer_height = surface->current_buffer->height;
	...
	if (state->args.show_indicator && state->auth_state != AUTH_STATE_IDLE) {
		...
		if (layout_text) {
			...
			new_height += fe.height + 2.0 * box_padding;
			if (extents.width > new_width) {
				new_width = extents.width;
			}
		}
	...
	if (buffer_width != new_width || buffer_height != new_height) {
		...
		surface->current_buffer->width = new_width;
		surface->current_buffer->height = new_height;
